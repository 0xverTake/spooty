...
class SpotDLClient:
    ...
    def get_track_metadata(self, track_id: str) -> Dict[str, Any]:
        try:
            ...
            logger.error(f'Failed to get track metadata: {str(e)}')
            return None

    def get_playlist_tracks(self, playlist_id: str) -> List[Dict[str, Any]]:
        try:
            playlist = self.spotify_client.get_playlist_tracks(playlist_id)
            return [{
                'id': item['track']['id'],
    ...
    def download_track(self, track_id: str, output_dir: str) -> str:
        try:
            track_url = f'spotify:track:{track_id}'
            song = self.spotdl.get_download_urls(track_url)[0]
            output_path = os.path.join(output_dir, f"{song.name}.mp3")
            self.spotdl.download_songs([song], output_path)
            return output_path
        except Exception as e:
...
def spotify_get_track_metadata(token, item_id):
    return spotdl_client.get_track_metadata(item_id)

def spotify_get_playlist_items(token, playlist_id):
    return spotdl_client.get_playlist_tracks(playlist_id)

def spotify_download_track(track_id, output_dir):
    return spotdl_client.download_track(track_id, output_dir)

def spotify_get_search_results(token, search_term, content_types):
    try:
        results = spotdl_client.spotify_client.search(search_term)
        tracks = []
...
# Fonctions supplémentaires optionnelles pour la compatibilité future
